...
export const FloatingNav = ({ navItems, className }: FloatingNavProps) => {
  ...
  const translatedNavItems = navItems.map((item) => {
    if (item.name === "About") return { ...item, name: t.nav.about };
    if (item.name === "Projects") return { ...item, name: t.nav.projects };
    if (item.name === "Cybersecurity") return { ...item, name: t.nav.cybersecurity };
    if (item.name === "Contact") return { ...item, name: t.nav.contact };
    return item;
  });
